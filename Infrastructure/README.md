...
Get-AzSAlert -AzureStackCredentials $credential -TenantID $AadTenant
```
...
```powershell
$credential = Get-Credential
Close-AzSAlert -AzureStackCredentials $credential -TenantID $AadTenant -AlertID "ID"
```
...
```powershell
$credential = Get-Credential
Get-AzSUpdateSummary -AzureStackCredentials $credential -TenantID $AadTenant
```
...
```powershell
$credential = Get-Credential
Get-AzSUpdate -AzureStackCredentials $credential -TenantID $AadTenant
```
...
```powershell
$credential = Get-Credential
Apply-AzSUpdate -AzureStackCredentials $credential -TenantID $AadTenant -vupdate "Update Version"
```
...
```powershell
$credential = Get-Credential
Get-AzSUpdateRun -AzureStackCredentials $credential -TenantID $AadTenant -vupdate "Update Version"
```
...
```powershell
$credential = Get-Credential
Get-AzSInfraRole -AzureStackCredentials $credential -TenantID $AadTenant
```
...
```powershell
$credential = Get-Credential
Get-AzSInfraRoleInstance -AzureStackCredentials $credential -TenantID $AadTenant
```
...
```powershell
$credential = Get-Credential
Get-AzSScaleUnit -AzureStackCredentials $credential -TenantID $AadTenant
```
...
```powershell
$credential = Get-Credential
Get-AzSScaleUnitNode -AzureStackCredentials $credential -TenantID $AadTenant
```
...
```powershell
$credential = Get-Credential
Get-AzSLogicalNetwork -AzureStackCredentials $credential -TenantID $AadTenant
```
...
```powershell
$credential = Get-Credential
Get-AzSStorageCapacity -AzureStackCredentials $credential -TenantID $AadTenant
```
...
```powershell
$credential = Get-Credential
Get-AzSStorageShare -AzureStackCredentials $credential -TenantID $AadTenant
```
...
```powershell
$credential = Get-Credential
Get-AzSIPPool -AzureStackCredentials $credential -TenantID $AadTenant
```
...
```powershell
$credential = Get-Credential
Get-AzSMacPool -AzureStackCredentials $credential -TenantID $AadTenant
```
...
```powershell
$credential = Get-Credential
Get-AzSGatewayPool -AzureStackCredentials $credential -TenantID $AadTenant
```
...
```powershell
$credential = Get-Credential
Get-AzSSLBMUX -AzureStackCredentials $credential -TenantID $AadTenant
```
...
```powershell
$credential = Get-Credential
Get-AzSGateway -AzureStackCredentials $credential -TenantID $AadTenant
```
...
```powershell
$credential = Get-Credential
Start-AzSInfraRoleInstance -AzureStackCredentials $credential -TenantID $AadTenant -Name "InfraRoleInstanceName"
```
...
```powershell
$credential = Get-Credential
Stop-AzSInfraRoleInstance -AzureStackCredentials $credential -TenantID $AadTenant -Name "InfraRoleInstanceName"
```
...
```powershell
$credential = Get-Credential
Restart-AzSInfraRoleInstance -AzureStackCredentials $credential -TenantID $AadTenant -Name "InfraRoleInstanceName"
```
...
```powershell
$credential = Get-Credential
Add-AzSIPPool -AzureStackCredentials $credential -TenantID $AadTenant -Name "PoolName" -StartIPAddress "192.168.55.1" -EndIPAddress "192.168.55.254" -AddressPrefix "192.168.0./24"
```
...
$Active=Get-AzSAlert -AzureStackCredentials $credential -TenantID $AadTenant|where {$_.state -eq "active"}

#Stop Infra Role Instance
Stop-AzSInfraRoleInstance -AzureStackCredentials $credential -TenantID $AadTenant -Name $Active.resourceName

#Start Infra Role Instance
Start-AzSInfraRoleInstance -AzureStackCredentials $credential -TenantID $AadTenant -Name $Active.resourceName

#Validate if error is resolved (Can take up to 3min)
Get-AzSAlert -AzureStackCredentials $credential -TenantID $AadTenant|where {$_.state -eq "active"}
```
