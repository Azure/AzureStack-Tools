...
To use these tools, first obtain Azure Stack compatible Azure PowerShell module. Unless you've installed from other sources, one way to do it is to obtain from public package repositories as follows. Note that both of these could still be used to operate against Azure as well as Azure Stack, but may lack some of the latest Azure features.

**For Azure Stack 1904 and higher**
...
2. Install and import the API Version Profile required by Azure Stack, into the current PowerShell session:
   ```powershell
...
**For all other Azure Stack versions**

Please follow the instructions at https://aka.ms/azspsh for the needed azure powershell.

## Download the Azure Stack Tools

### Download the tools via compressed archive from a GitHub release tag

See the available version release tags at https://github.com/Azure/AzureStack-Tools/tags.

### Download the tools by cloning the Git repository

Using the Git Bash console, for example:

```commandline
# For the current Azure Stack version:
...
Using the PowerShell console, for example:
