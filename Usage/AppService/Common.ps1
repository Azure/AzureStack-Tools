...
function Get-AppServiceBillingRecords {
  ...
  "Fetching Usage Records from ARM endpoint ..." | Write-Host -ForegroundColor Cyan
  $result = Get-AzResource @params -ErrorVariable RestError -Verbose

  if ($RestError) {
    return
  }

  "Fetched {0} Usage Records" -f $result.count | Write-Host -ForegroundColor Green

  "Processing Usage Records ..." | Write-Host -ForegroundColor Cyan
  $usageSummary = [System.Collections.Generic.List[System.Object]]::new()
  $count = $result.Count
  $index = 0
  $result  | ForEach-Object {
    $index++
    $status = "Processing Record $index of $count"
    Write-Progress -Activity "Processing Usage Records" -Status $status -PercentComplete ($index / $count * 100)
    if ($meters.ContainsKey($_.Properties.MeterId)) {
      ...
      $usageSummary.Add($record) | Out-Null
    }
  }
