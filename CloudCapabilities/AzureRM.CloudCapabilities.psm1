...
function Get-AzureRMCloudCapability() {
    ...
    foreach ($providerNamespace in $providerNamespaces) {
        ...
        try {
            $resourceTypes = (Get-AzureRmResourceProvider -ProviderNamespace $providerNamespace -ErrorAction Stop | ? {$_.Locations -contains $location -or $_.Locations -contains "global"}).ResourceTypes
